...
// 1) PRE-SUBMIT: apri conferma, non inviare ancora
function preSubmit (e) {
  ...
  if (honeypot.value && honeypot.value.trim() !== '') {
    // bot: non inviamo nulla e “simuliamo” successo
    success.value = true
    email.value = ''
    name.value = ''
    message.value = ''
...
// 2) CONFERMA: invia davvero su Firestore
...
<template>
  ...
  <section class="contact-form-section" aria-labelledby="contact-form-title">
    <h2 id="contact-form-title">Per lavori su commissione, collaborazioni o altro</h2>
    <p class="form-lead">Compila il form qui sotto, ti risponderò il prima possibile!</p>
  ...
  <!-- MODAL di conferma (con riepilogo) -->
  <div
    v-if="showConfirm"
    class="modal-overlay"
    role="dialog"
...
.hero-image-container {
  ...
  background-position: right center; transform: translateY(-8%);
}
body.dark-mode .hero-image-container { background-image: url('/images/contacts/contacts_darkmode.png'); }
.header-content-wrapper {
  position: absolute; top: 50%; left: 0; right: 0; transform: translateY(-50%);
  text-align: center; width: 100%; padding-inline: var(--margin-desktop); box-sizing: border-box;
}
...
@media (max-width: 1024px) and (min-width: 769px) {
  .hero-image-container { background-position: right center; transform: translateY(-10%); opacity: 0.95; }
}
@media (max-width: 768px) {
  .hero-image-container { background-position: right top; transform: translateY(-10%); opacity: 0.9; }
}

/* ===== FORM (stile invariato) ===== */
.contact-form-section {
  background-color: color-mix(in srgb, var(--color-accent) 10%, transparent);
  max-width: 760px; margin: 1rem auto 7rem; padding: 2rem;
  border: 1px solid var(--color-accent);
}
.contact-form-section h2 {
  text-align: center; margin: 0 0 0.5rem;
  font-size: clamp(18px, 2vw, 22px); color: var(--color-accent);
}
...
input, textarea {
  ...
  font: 400 16px/1.4 var(--font-body); background: var(--color-surface); color: var(--color-text);
}
...
.primary-btn {
  align-self: center; background-color: var(--color-accent); color: var(--color-surface);
  font-weight: 700; font-size: 14px; padding: 10px 18px; border: 1px solid currentColor; border-radius: 0;
...
@media (max-width: 768px) {
  .contact-form-section { margin: 1.5rem auto 5rem; padding: 1.25rem; }
...
.modal-btn.cancel:hover  { background: rgba(0,0,0,.06); }
</style>
