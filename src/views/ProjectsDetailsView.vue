...
/* ==========================================================================
   Immagini per lo stage (Immagine centrale grande)
   ========================================================================== */
const images = computed(() => {
  ...
  // 1. Identifichiamo la cover originale (1200x800)
  const coverRaw = (project.value.main_image?.trim?.() || project.value.img || '').trim()
  
  if (isImgUrl(coverRaw)) {
    const coverKey = normKey(coverRaw)
    
    // CERCHIAMO: Esiste un file con lo stesso nome nella galleria (1920x1080)?
    const hiResMatch = galleryPairs.value.find(p => normKey(p.hi) === coverKey)
    
    // Se esiste la versione HD, usiamo quella (p.hi), altrimenti usiamo la coverRaw
  ...
  // 2. Aggiungiamo le altre immagini p.hi (1920x1080) saltando i duplicati
      ...
      <section class="meta" aria-label="Dettagli progetto">
        ...
        <div class="col">
          ...
          <p v-if="project.description" class="desc">
            {{ project.description }} <div v-if="project.behance_url" class="behance-header-link">
...
.behance-header-link {margin-bottom: 30px; }
.a-behance-link { 
  display: inline-flex; 
  align-items: center; 
  gap: 8px; 
...
.behance-mini-icon { width: 16px; height: 16px; }
.behance-header-link a:hover {
